...
const OnePlan = () => {
 ...
 useEffect(() => {
   ...
   return () => clearInterval(imageslideshowInterval);
 }, [images]);
  const property = {
    id: 4,
    buildingPlan: "Condo",
    ...
    <div className="w-full h-full overflow-y-auto overflow-x-hidden p-5">
      ...
      >
        ...
        <div className="flex justify-between my-3">
          <button
            ...
            className="px-4 py-2 rounded-md bg-gray-200 border-blue-500 border-2 text-white"
          >
            Cancel
          </button>
          <button
      ...
      {loading ? (
        ...
        <div className="p-3">
          ...
          <div className="flex flex-col-reverse sm:flex-row gap-2 my-6">
            {images.length > 0 && (
              <img
                src={images[currentImageIndex]}
                alt=""
                className="w-[100%] sm:w-[40%] rounded-lg h-[30vh] sm:h-[50vh]"
              />
            )}
            <div className="flex gap-3 justify-center items-center sm:w-[40%] ">
              <p className="text-gray-400 text-opacity-50 text-sm">Actions:</p>
              ...
              <button
                onClick={openModal}
                className="flex gap-2 items-center border-2 rounded-lg h-fit px-4 py-2"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="18"
          ...
          <div className="flex flex-col gap-4">
            <div className="flex gap-4 ">
              <p className="text-gray-400 text-opacity-50 text-sm">Title:</p>
              <p className="font-semibold ">{project?.category.title}</p>
            </div>
            <div className="flex gap-4">
              <p className="text-gray-400 text-opacity-50 text-sm">Price:</p>
              <p className="font-semibold text-[#898989]">
                ${project?.planPrice}
              </p>
            </div>
            <div className="flex gap-4">
              <p className="text-gray-400 text-opacity-50 text-sm">Location:</p>
              <p className=" ">{project?.location}</p>
            </div>
            <div className="flex flex-col gap-4">
              <p className="text-gray-400 text-opacity-50 text-sm">
                Uploaded by:
              </p>
              <p className=" ">{project?.uploadedBy.fullName}</p>
            </div>

            <p className="font-semibold my-2">More details</p>
            <div className="w-[50%] gap-4 flex flex-col sm:flex-row justify-around">
              ...
              <div className="flex gap-2 items-center">
                <p className="text-gray-400 text-opacity-50 text-sm">Floors:</p>
                <img src="/svg/room.svg" alt="" />
                <p>{project?.numberOfFloors}</p>
              </div>
              <div className="flex gap-2 items-center">
                <p className="text-gray-400 text-opacity-50 text-sm">Living:</p>
                <img src="/svg/room.svg" alt="" />
                <p>{project?.livingRooms}</p>
              </div>
            </div>
          </div>
        </div>
